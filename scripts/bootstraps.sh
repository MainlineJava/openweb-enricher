...
# also make this file executable (in case it's saved as bootstraps.sh)
chmod +x "$0" || true
...
if command -v systemctl >/dev/null 2>&1; then
  ...
  if [[ "$install_svc" =~ ^[Yy] ]]; then
    DEFAULT_USER="$(whoami)"
    read -p "Service should run as which user? [${DEFAULT_USER}]: " SERVICE_USER
    SERVICE_USER="${SERVICE_USER:-$DEFAULT_USER}"
    SERVICE_GROUP="$(id -gn "$SERVICE_USER" 2>/dev/null || echo "$SERVICE_USER")"
    SERVICE_PATH="/etc/systemd/system/openweb_enricher.service"
    echo "Creating systemd unit at $SERVICE_PATH running as ${SERVICE_USER}:${SERVICE_GROUP}"
...
if command -v systemctl >/dev/null 2>&1; then
  echo " - If you installed the systemd service: sudo systemctl status openweb_enricher.service"
fi